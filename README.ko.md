...
예시 하나 (보장값이 아니다): 이 커널 위에 만든 비공개 PPO 파이프라인(정책 1024×2 MLP, 두 기체
에피소드 약 65,500 개 병렬, 이터당 결정 96 스텝 × 6 프레임, 에폭 4, 같은 RTX 5070 Ti)에서 물리를
torch 백엔드에서 커널로 바꾸자 학습 한 이터가 약 19.0 초에서 약 15.6 초가 됐다.  이제 물리는
그 이터의 약 26 ms (0.2 %) 다.  병목이 학습 쪽으로 옮겨 가고, 시뮬레이터 비용은 사실상 사라진다.
[자세히](docs/benchmarks.md#simulator-vs-end-to-end-training).
