...
**JSBSim 의 F-16 을 손으로 짠 CUDA 커널로: GPU 한 장에서 초당 29.5 억 기체·프레임, JSBSim 과
한 프레임씩 대조.**
...
v0.2.1, RTX 5070 Ti, Ryzen 9 9950X (16 코어 / 32 스레드).  float32, 호출당 6 프레임, GPU 는 다른
작업 없음 ([조건과 전체 숫자](docs/benchmarks.md)):

- CUDA 커널 262,144 대에서 **초당 29.5 억 기체·프레임** -- GPU 한 장으로 **F-16 2,400 만 대를
  실시간으로** 날릴 수 있는 양이다.
- **JSBSim CPU 1 코어의 24,100 배, 하드웨어 스레드 32 개 전부의 1,240 배** (CUDA 커널 대
  JSBSim `run()`, 물리만).
...
| torch CPU 백엔드, 16,384 대, 스레드 16 | 0.86 M | 1.2 µs | 7.0 배 |
| torch GPU 백엔드 + CUDA 그래프, 262,144 대 | 33.8 M | 30 ns | 277 배 |
| **CUDA 커널, 262,144 대** | **2.95 G** | **0.34 ns** | **24,100 배** |

비용 = 1 / 처리량 (배치 전체에 나눠 낸 값).  20 Hz 결정 한 스텝(6 프레임)은 그 6 배다:
기체 한 대당 JSBSim 1 코어로 49 µs, 커널로 2.0 ns.
...
예시 하나 (보장값이 아니다, 이전 빌드로 잼) -- 같은 PC 에서 같은 비공개 PPO 학습기(정책 256×2 MLP)를 처음엔
