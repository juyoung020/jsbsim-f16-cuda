...
5. **Adams-Bashforth history slot `[0]` is the previous frame.**  JSBSim integrates
   u, v, w with AB2 and position with AB3.  When seeding a mid-flight state, filling slots
   `[1]`, `[2]` only is 90 times worse than not seeding at all.

## Timing within a frame

6. **Controls act one frame late.**  A JSBSim frame starts with the integrator
...
7. **The FLCS delays itself by one frame.**  Several of its inputs (alpha, rates, pilot
...
8. **Pilot load factor mixes ages**: last frame's specific force and angular
   acceleration, this frame's angular rate.  The other combinations agree at the median
   and disagree by up to 4 g at high load.
9. **Mass is evaluated with the fuel burned up to the previous frame**
   ...
   with it, 3e-11.
10. **The first frame after a trim snaps the engine** (N2 and fuel flow jump to the
    commanded value — JSBSim's trim step), then normal spooling resumes.

## Engine and fuel

11. **Fuel flow is rate-limited** (`FGTurbine::Seek`: +5,000 / −10,000 pph/s).  Thrust
...
12. **Throttle is applied once.**  The FLCS output `fcs/throttle-pos-norm` is already
...
13. **Calibrated airspeed**: impact pressure uses the local static pressure, the inverse
...
14. **The trailing-edge flap initial value is a switch, not a table.**  Interpolating it
